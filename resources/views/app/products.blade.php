                    ...
                    <div class="bg-white rounded-lg shadow-sm p-4">
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <button type="button" onclick="clearAllFilters()"
                                class="text-blue-600 text-xs font-medium">ล้างทั้งหมด</button>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <div class="relative">
                                ...
                                <div id="brand-dropdown"
                                    class="hidden absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                                    <button type="button" onclick="setBrand('')"
                                        class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100">ทั้งหมด</button>
                                    @foreach ($brands as $brand)
                                        <button type="button" onclick="setBrand('{{ $brand }}')"
                                            class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100">{{ $brand }}</button>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="relative">
                                ...
                                <div id="price-dropdown"
                                    ...
                                    <button type="button" onclick="setPriceRange('', '')"
                                        class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 {{ !request('min_price') && !request('max_price') ? 'bg-blue-50 text-blue-600' : '' }}">
                                        ทุกราคา
                                    </button>
                                    <button type="button" onclick="setPriceRange('0', '10000')"
                                        class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 {{ request('min_price') == '0' && request('max_price') == '10000' ? 'bg-blue-50 text-blue-600' : '' }}">
                                        ต่ำกว่า 10,000 บาท
                                    </button>
                                    <button type="button" onclick="setPriceRange('10000', '20000')"
                                        class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 {{ request('min_price') == '10000' && request('max_price') == '20000' ? 'bg-blue-50 text-blue-600' : '' }}">
                                        10,000 - 20,000 บาท
                                    </button>
                                    <button type="button" onclick="setPriceRange('20000', '30000')"
                                        class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 {{ request('min_price') == '20000' && request('max_price') == '30000' ? 'bg-blue-50 text-blue-600' : '' }}">
                                        20,000 - 30,000 บาท
                                    </button>
                                    <button type="button" onclick="setPriceRange('30000', '50000')"
                                        class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 {{ request('min_price') == '30000' && request('max_price') == '50000' ? 'bg-blue-50 text-blue-600' : '' }}">
                                        30,000 - 50,000 บาท
                                    </button>
                                    <button type="button" onclick="setPriceRange('50000', '100000')"
                                        class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 {{ request('min_price') == '50000' && request('max_price') == '100000' ? 'bg-blue-50 text-blue-600' : '' }}">
                                        50,000 - 100,000 บาท
                                    </button>
                                    <button type="button" onclick="setPriceRange('100000', '')"
                                        class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 {{ request('min_price') == '100000' && !request('max_price') ? 'bg-blue-50 text-blue-600' : '' }}">
                                        มากกว่า 100,000 บาท
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-4">
                        <div class="flex items-center justify-between">
                            ...
                            <select name="sort" onchange="submitForm()"
                                class="text-sm text-blue-600 font-medium bg-transparent border-none focus:ring-0">
                                <option value="featured"
                                    {{ request('sort', 'featured') === 'featured' ? 'selected' : '' }}>แนะนำ</option>
                                <option value="price_low" {{ request('sort') === 'price_low' ? 'selected' : '' }}>ราคา:
                                    น้อย → มาก</option>
                                <option value="price_high" {{ request('sort') === 'price_high' ? 'selected' : '' }}>ราคา:
                                    มาก → น้อย</option>
                ...
                @if ($products->count() === 0)
                    ...
                    <div class="text-center py-8">
                        <svg class="w-16 h-16 mx-auto text-gray-300 mb-4" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.347 0-4.515.862-6.172 2.172M12 3v12">
                            </path>
                        </svg>
