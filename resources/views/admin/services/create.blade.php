...
@section('content')
    <div class="px-4 py-6 sm:px-0">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">เพิ่มบริการใหม่</h1>
            <p class="mt-1 text-sm text-gray-600">สร้างบริการใหม่สำหรับลูกค้า</p>
        </div>
        ...
        <form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <div class="mb-6">
                    ...
                    <select name="icon_color" id="icon_color"
                        ...
                        <option value="blue" {{ old('icon_color') === 'blue' ? 'selected' : '' }}>น้ำเงิน</option>
                        <option value="green" {{ old('icon_color') === 'green' ? 'selected' : '' }}>เขียว</option>
                        <option value="orange" {{ old('icon_color') === 'orange' ? 'selected' : '' }}>ส้ม</option>
                        <option value="red" {{ old('icon_color') === 'red' ? 'selected' : '' }}>แดง</option>
                        <option value="purple" {{ old('icon_color') === 'purple' ? 'selected' : '' }}>ม่วง</option>
                        <option value="yellow" {{ old('icon_color') === 'yellow' ? 'selected' : '' }}>เหลือง</option>
                    </select>
                    @error('icon_color')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="price_display" id="price_display" value="{{ old('price_display') }}"
                        step="0.01" min="0"
                        class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    @error('price_display')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <div id="packages-container">
                    <div class="package-item border rounded-lg p-4 mb-4" data-index="0">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-md font-medium text-gray-900">แพ็กเกจ 1</h3>
                            <button type="button" onclick="removePackage(0)"
                                class="text-red-600 hover:text-red-800 text-sm hidden">ลบ</button>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">ชื่อแพ็กเกจ <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="packages[0][name]" value="{{ old('packages.0.name') }}"
                                    class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                    required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">ราคา <span
                                        class="text-red-500">*</span></label>
                                <input type="number" name="packages[0][price]" value="{{ old('packages.0.price') }}"
                                    step="0.01" min="0"
                                    class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                    required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">หน่วย <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="packages[0][unit]" value="{{ old('packages.0.unit') }}"
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea name="packages[0][description]" rows="3"
                                    class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                    required>{{ old('packages.0.description') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <div id="details-container">
                    <div class="detail-item flex items-center gap-4 mb-4" data-index="0">
                        <input type="text" name="details[0]" value="{{ old('details.0') }}"
                        ...
                        <button type="button" onclick="removeDetail(0)"
                            class="text-red-600 hover:text-red-800 text-sm hidden">ลบ</button>
                    </div>
                </div>
